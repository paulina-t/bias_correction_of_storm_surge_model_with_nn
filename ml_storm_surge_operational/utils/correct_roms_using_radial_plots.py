...
class CorrectROMS():
    ...
    def setup(self):
        ...
        hlp.dump_df_with_pkl(self.df, self.df_path)
        del self.df
            
        ensemble_variables = ['stormsurge', 'stormsurge_corrected', 'bias']
        if self.use_existing_dataframes:
            for m in self.ensemble_members:
                if m==-1:
                    m=52
                df_m_path = self.make_df_m_path(m)
                file_m = os.path.isfile(df_m_path)
                if file_m:
                    print('DataFrame for ensemble member ' + str(m) + 'already exists.')
                else:
                    prep_m = PrepareDataFrames(
                        variables = list(set(self.variables) & set(ensemble_variables)),
                        stations = self.stations,
                        ensemble_members=[m],
                        datetime_start_hourly = self.datetime_start_hourly,
                        datetime_end_hourly = self.datetime_end_hourly,
                    ...
                    df_m = prep_m.prepare_features_labels_df()
                    
                    df_m_path = self.make_df_m_path(m)
                    hlp.dump_df_with_pkl(df_m, df_m_path)
                    del df_m
        else: # If not using existing dataframes            
            print('Pickle dataframe for each ensemble member.') 
            for m in self.ensemble_members:
                if m==-1:
                    m = 52
                prep_m = PrepareDataFrames(
    ...
    def make_df_m_path(self, m):
        if m==None:
             m = 52
        path = (
            '/lustre/storeB/project/IT/geout/machine-ocean/workspace/paulinast'
            + '/storm_surge_results/tmp_df_'
            + str(m)
            +'.pkl'
            )
        return path
        
    def open_ensemble_data(m, variables=['stormsurge_corrected', 'bias']):
    ...
    def create_lookup_table(self, test_var, train_var, plot_var, interpolation_method=None):
        """
        ...
        if interpolation_method is not None:  #Pressure plots do not use interp method
        ...
        lookup_table['rmse']['rmse_'+ hlp.crop_strings(test_var) + '_' + correction_str + '_' + interpolation_method] = {}
        lookup_table['rmse']['rmse_' + hlp.crop_strings(test_var)] = {}
        
        
        for station in self.stations:
            lookup_table['data'][correction_str][station] = {}
            lookup_table['rmse']['rmse_'+ hlp.crop_strings(test_var) + '_' + correction_str + '_' + interpolation_method][station] = {}
    ...
    def correct_using_radial_plots(self,  aggregated_dict_with_corrections, sorted_dict_to_correct,
        ...
        for station in self.stations:  
            ...
            #corr_col = 'correctionRP_' + station 
            ...
            for m in members:
                ...
                for h in range(horizon_for_loop + 1):
                    ...
                    if (('t' + str(h)) in sorted_dict_to_correct[fun]['test'][station][m_str].keys()) and (('t' + str(h)) in aggregated_dict_with_corrections[fun]['train'][station][m_str].keys()):
    
                        # ------ DF with correction from train set aggregated by bins ------
                        aggregated_df = copy.deepcopy(
                            aggregated_dict_with_corrections[fun]['train'][station][m_str]['t' + str(h_aggreg)][corr_col].fillna(0)
                            )
                        
                        # ------ DF with data to correct from test set sorted by bins ------
                        sorted_df = copy.deepcopy(
                            sorted_dict_to_correct[fun]['test'][station][m_str]['t' + str(h)]
                            )
                        ...
                        if h == 0:
                            ...
                            self.lookup_table['data']['correctionRP'][station][m_str] =  (
                                aggregated_df
                                .to_numpy()
                                )
                    ...
                    self.lookup_table['rmse'][rmse_var][station][m_str] = rmse_corrected
                    self.lookup_table['rmse']['rmse_' + hlp.crop_strings(test_var)][station][m_str] = rmse_not_corrected
    ...
    def initiate_dict(self):
        ...
        for dataset in ['train', 'test']:
            ...
            for station in self.stations:
                ...
                # When using hindcast variable, m == ''
                my_dict['mean'][dataset][station]['m'] = {}  
                for m in self.ensemble_members:
                    my_dict['mean'][dataset][station]['m' + str(m)] = {}
        return my_dict

    def make_col_names(self, variables, station, m, h):
        col_names = []
        m_str = ''
        ...
        for var in variables:  
            if var == 'stormsurge' or var =='stormsurge_corrected':
                col = var + m_str + h_str + '_' + station
            else:
                col = var + h_str + '_' + station
            col_names.append(col)
        return col_names
    
    def make_df_m_h(self, var1, var2, m, h, station, aggregation_var):
        """
        Load data with pickle and make dataframe with data for ensemble member
        m and horizon h.
        ...
        -------
        df_m_h : DataFrame
            DataFrame containing only the variables needed for calling running 
            radial plots or pressure plots with eda.py for m and h.

        """
        columns_1 = self.make_col_names([var1], station, m, h)
        print('columns_1: ', columns_1)
        columns_2 = self.make_col_names([var2], station, m, h)
        print('columns_2: ', columns_2)
        if aggregation_var == 'wind':
            columns_2.append('wind_speed_' + station)
            columns_2.append('wind_dir_' + station)
        elif aggregation_var == 'pressure':
            columns_2.append('msl_' + station)
            
        if var1 == self.roms_oper_var and m!='':
            df_m_h_1 = hlp.load_and_subset_df_with_pkl(self.make_df_m_path(m), columns_1)  # TODO: Check if it is possible to loop over m inside h-loop so that I don't need to load the same df multiple times
            df_m_h_2 = hlp.load_and_subset_df_with_pkl(self.df_path, columns_2)
            df_m_h = pd.concat([df_m_h_1, df_m_h_2], axis=1)
            del df_m_h_1
            del df_m_h_2  
        else:
            df_m_h = hlp.load_and_subset_df_with_pkl(self.df_path, columns_1 + columns_2)
        return df_m_h
    ...
    def get_data_and_make_radial_plots(self, fun, var1:str, plot=True):
        ...
        for station in self.stations:                  
            ...
            for m in self.ensemble_members:
                if var1 == self.roms_oper_var:
                    if m == 52 or m == -1:
                        var1_m = self.roms_oper_var
                    elif m < 52 and m >= 0:   
                        var1_m = self.roms_oper_var + '_m' + str(m)
                    elif m < -1:  # When counting from the last position in a vector
                        m = 53 + m
                        var1_m = self.roms_oper_var + '_m' + str(m)
                else:
                    var1_m = self.roms_hind_var
                    m = ''
                
                if first_m:
                    # --- Loop over horizons ---
                    for h in range(horizon + 1):
                        df_m_h = self.make_df_m_h(
                            var1, 
                            var2, 
                            m, 
                            h, 
                            station,
                            aggregation_var='wind'
                            )
                        df_train, df_test = hlp.split_df(df_m_h, self.datetime_split)
                        del df_m_h
                        ...
                        if fun == 'mean':
                                ...
                                if m:
                                    m_str = 'm' + str(m)
                                print('m_str: ', m_str)
                                aggregated_dict['mean'][dataset][station][m_str]['t' + str(h)] = aggregated_df
                                sorted_dict['mean'][dataset][station][m_str]['t' + str(h)] = sorted_df
     
                if var1_m == self.roms_hind_var:
                    first_m = False
                ...
                if first_m:
                    ...
                    for h in range(horizon + 1):
                        ...
                        for df_to_plot in [df_train, df_test]:
                            ...
                            if fun == 'mean':
                                aggregated_dict['mean'][dataset][station]['m' + str(m)]['t' + str(h)] = aggregated_df
                                sorted_dict['mean'][dataset][station]['m' + str(m)]['t' + str(h)] = sorted_df
        ...
        for station in self.stations:
            ...
            for m in members:
                ...
                for h in range(horizon_for_loop + 1):
                    ...
                    print("sorted_dict_to_correct[fun]['test'][station][m_str].keys(): ", sorted_dict_to_correct[fun]['test'][station][m_str].keys())
                    print("aggregated_dict_with_corrections[fun]['train'][station][m_str].keys(): ", aggregated_dict_with_corrections[fun]['train'][station][m_str].keys())
                    if (('t' + str(h)) in sorted_dict_to_correct[fun]['test'][station][m_str].keys()) and (('t' + str(h)) in aggregated_dict_with_corrections[fun]['train'][station][m_str].keys()): # Otherwise there might not be enough test data and nothing to correct
                        # ------ DF with correction from train set aggregated by bins ------
                        aggregated_df = copy.deepcopy(
                            aggregated_dict_with_corrections[fun]['train'][station][m_str]['t' + str(h_aggreg)][corr_col].fillna(0)
                            )
                        ...
                        if h == 0:
                            # Add level in dict for hz
                            self.lookup_table['data']['correctionPP'][station][m_str] =  (
                                aggregated_df
                                .to_numpy()
                                )
