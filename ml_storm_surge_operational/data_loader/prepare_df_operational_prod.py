...
from ml_storm_surge_operational.data_loader.read_kyststasjoner_hourly_prod import (
    ReadKyststasjoner 
    as ReadKyststasjonerHourly
)
from ml_storm_surge_operational.data_loader.read_forcings import ReadForcings
import time
...
class PrepareDataFrames():
    ...
    def generate_station_forcing_df(self, variables):
        ...
        df_station_forcing = self.generate_forcings_df(variables)
        print('generate_arome_df should be updated with new paths')
        return df_station_forcing

    
    def generate_forcings_df(self, variables):
        # Generate a DataFrame with weather forecasts.
        print('Generate Forcings df...')

        # Define the actual times of the final DataFrame
        rf_small = ReadForcings(
            self.datetime_start_hourly, 
            self.datetime_end_hourly
            )   
        times_small = rf_small.make_time_idx()    
        
        # Use the following piece of code if you do not need past forecasts
        # The logic is similar to that in generate_kyststasjoner_df, but without
        # past forecasts.
        path_list = rf_small.make_path_list_ppi()
        first_iter = True
        for station in self.oper_stations:
            for var in variables:
                small_data_array = rf_small.make_data_arrays(
                    [var], 
                    [station], 
                    path_list, 
                    self.horizon
                ...
                col_names = rf_small.make_hz_col_names(
                    [var], 
                    [station], 
                    self.horizon, 
                    fhr=0
                ...
                if first_iter:
                    ...
                    df_forcings = pd.concat([df_forcings, df], axis=1)
        return df_forcings
    ...
    def generate_kyststasjoner_df(self, variables):
        ...
        """ 
        # Generate a DataFrame with ROMS forecasts.
