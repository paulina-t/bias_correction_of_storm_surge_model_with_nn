...
class ReadKyststasjoner():
    ...
    !!! There are many methods here that are not used in production and can be
    removed.
    """
    ...
    def __init__(self, 
                 ...
                 ensemble_member=-1,
                 data_dir:str='/lustre/storeB/project/metproduction/products/util/vannstand/'):
    ...
    """def make_data_arrays(self, var, station_name, path_list, hz):
        ...
        for i in range(l):
            data_file_i = xr.open_dataset(path_list[i], engine='netcdf4')
            station_nr = self.get_station_id(data_file_i, station_name)
            for j in range(12):  # For each file, fill in the next 12 hours
                try:
                    length = j + hz
                    if length > 121:  # TODO: change this maximum!!!! # We have only 120 preditions
                        length = 121
                    ...
                    if var == 'stormsurge_corrected' or var == 'stormsurge':
                        data[i * 12 + j, :length-j] = ( 
                            ...
                            .isel(
                                {
                                'station':station_nr, 
                                'time':slice(j * 6, length, 6),  # J * 6 because we have 10 min data in the new files
                        ...
                        if station_nr == 1:
                            ...
                            print('i:', i, 'j:', j, 'length:', length, 'var:', var)
                            #print('data[i * 12 + j, :length-j]:', data[i * 12 + j, :length-j])
                            print(data_file_i[var]
                                  .isel(
                                      {'station':station_nr, 
                                    'time':slice(j * 6, length, 6), 
                                    'dummy':0}
    ...
    def make_data_arrays(self, var, station_name, path_list, hz):
        ...
        for i in range(l):
            try:
                ...
                for j in range(12):  # For each file, fill in the next 12 hours
                    start = j * 6  # 6 time steps per hour for 10-minute data
                    end = start + hz * 6
                    if end > 121 * 6:  # 121 is the max number of hourly steps
                        end = 121 * 6

                    if var == 'stormsurge_corrected' or var == 'stormsurge':
                        data[i * 12 + j, : (end - start) // 6] = (
                            data_file_i[var]
                            .isel(
                                {
                                    'station': station_nr,
                                    'time': slice(start, end, 6),  # Select only hourly data
                                    'ensemble_member': self.ensemble_member,
                                    'dummy': 0
                                }
                            )
                        ...
                        if station_nr == 1:
                            print('var:', var)
                            print('path_list[i]:', path_list[i])
                            print('i:', i, 'j:', j, 'start: ', start, 'end: ', end, 'var:', var)
                            #print('data[i * 12 + j, :length-j]:', data[i * 12 + j, :length-j])
                            print(data_file_i[var]
                                  .isel(
                                      {'station':station_nr, 
    ...
    def fill_next_11_hrs(self, data):
        ...
        for i in range(11):
            data_i = data.isel(horizon=slice(i+1, len_hz, 6)) #i+1 :
