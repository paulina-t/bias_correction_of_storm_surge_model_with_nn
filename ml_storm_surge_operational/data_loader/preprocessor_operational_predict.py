...
# TODO: Define the methods if the verbose is True -> evaluate verbose only once
...
class PreprocessInput():
    ...
    def __init__(
            ...
            horizon:int = 61,  # list with the same length as label_vars
    ...
    def preprocess_data(self):
        ...
        self.x, self.y = self.remove_rows_with_nans(
            self.features,
            self.labels
            )
    ...
    def generate_feature_array_future(self):
        ...
        verboseprint1(
            self.verbose,
            'Generating feature array for future times...'
            )
        count_station = 0
        variable_names = []
    ...
    def generate_feature_array_past(self):
        ...
        for station in self.feature_stations:
            ...
            for var in self.feature_vars:
                ...
                for h in range(-H, 0): # Kyststatsjoner data starts counting at 0, but it is actually the first lead time.
                    if h != 0:
                        hz_str = '_t' +str(h)
                    vname = var + hz_str + '_' + station
    ...
    def concatenate_arrays(self):
        ...
        if ('features_past' in locals()) and ('features_future' in locals()) :
            features = np.concatenate((features_past, features_future), axis=1)  # TODO: Check values!!
    ...
    def imputation(self):    
        ...
        OBS! Not running the imputation here. Since we have only one record, if 
        data is missing, we do not run the ML models.
        """
        verboseprint1(self.verbose, 'Performing imputation...')
        if self.imputation_values:             
            try:
                self.x[np.isnan(self.x)] = self.imputation_values[
                    np.isnan(self.x)
                    ] 
            except:
    ...
    def normalize_arrays(self):
        ...
        if self.normalize:
            x_y_dir = (
                ...
                + '/paulinast/operational/data/x_y/' 
                + self.label_station
                )

            x_y_file = 'x_y_' + self.label_station + '.pickle'
            with open(x_y_dir + '/' + x_y_file, 'rb') as handle:
                b = pickle.load(handle)

            # Load average and standard deviation of the training data
            x_train_mean = b['x_train_mean']
            x_train_std = b['x_train_std']
